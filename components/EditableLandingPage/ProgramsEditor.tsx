    ...
    <section className="mb-8">
      ...
      {localItems.map((item, index) => (
        <div key={item.id || index} className="border p-4 rounded mb-4">
          ...
          {/* Program Image Uploader Section */}
          <div className="mt-2">
            <ProgramImageUploader
              onUpload={(url) => handleChange(index, 'landingImage', url)}
              publicId={item.id ? `program-${item.id}` : `program-temp-${index}`}
            />
          </div>
          <div>
...
/* --------------------------------------------------------------------------
   PROGRAM IMAGE UPLOADER COMPONENT
   --------------------------------------------------------------------------
   This component provides a drag & drop area, a file input, and a URL input.
   It uses your /api/main/landing/programs/upload endpoint to update the image.
   A publicId prop is provided so that each program always uploads to the same Cloudinary public ID,
...
const ProgramImageUploader: React.FC<ProgramImageUploaderProps> = ({ onUpload, publicId }) => {
