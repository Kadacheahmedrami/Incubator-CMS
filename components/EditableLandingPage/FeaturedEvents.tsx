...
const FeaturedEventsSelector: React.FC = () => {
  const [entries, setEntries] = useState<Event[]>([]);
  const [featuredRecords, setFeaturedRecords] = useState<FeaturedEvent[]>([]);
  ...
  const fetchFeaturedRecords = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/main/landing/featured-events');
  ...
  const handleFeatureSelected = async () => {
    ...
    if (selected.size === 0) {
      alert("Please select at least one event to feature");
      return;
    }
    try {
      setLoading(true);
      for (const entryId of selected) {
        ...
        if (!res.ok) {
          ...
          throw new Error(errData.error || 'Failed to feature event');
        }
      }
      setSuccessMsg("Selected events have been featured successfully.");
      setSelected(new Set());
  ...
  const handleUnfeature = async (featuredId: number) => {
    try {
      setLoading(true);
      const res = await fetch(`/api/main/landing/featured-events/${featuredId}`, {
        method: 'DELETE',
      });
      if (!res.ok) throw new Error('Failed to unfeature event');
      await fetchFeaturedRecords();
      setSuccessMsg("Event has been unfeatured successfully.");
      setLoading(false);
    } catch (err: any) {
      setError(err.message);
      setLoading(false);
  ...
  const featuredEntryIds = featuredRecords.map((fr) => fr.eventId);
  ...
  const getFeaturedRecordId = (entryId: number): number | null => {
    const record = featuredRecords.find((fr) => fr.eventId === entryId);
    return record ? record.id : null;
  };

  return (
    <div className="container mx-auto p-8 space-y-8">
      <h2 className="text-3xl font-bold text-center">Manage Featured Events</h2>
      ...
      <section>
        <h3 className="text-2xl font-semibold mb-4">Available Events</h3>
        {nonFeaturedEntries.length === 0 ? (
          <p>All events are featured.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
...
export default FeaturedEventsSelector;
